...
- Voice: `en-GB-SoniaNeural` unless the pair profile overrides it
...
- `Brightness 80 percent`, `make brighter`, `make dimmer`
- `Volume 55 percent`, `louder`, `softer`
- `Display off`, `screen on`, `go to sleep`, `wake up`
- `Battery`, `Temperature`, `Sensors`, `Proximity`, `Side`, `Motion`
...
```json
{
  "reply": "I will read this message on StackChan.",
  "actions": [
...
{
  "action": "image_search",
  "query": "aurora borealis over iceland",
...
```json
{
  ...
  "prompt": "Briefly describe what you see in English.",
...
Publishes to `hermes-stackchan/jeeves/cmd/display`.
...
Publishes to `hermes-stackchan/jeeves/cmd/face`.
...
Publishes to `hermes-stackchan/jeeves/cmd/move`.
...
Publishes to `hermes-stackchan/jeeves/cmd/motion`.
...
Publishes to `hermes-stackchan/jeeves/cmd/led`.
...
Publishes to `hermes-stackchan/jeeves/cmd/device`.

Supported fields: `volume_pct`, `brightness_pct`, `display_sleep`, `display_wake`.

### sound

Publishes to `hermes-stackchan/jeeves/cmd/sound`.

Currently supports simple local tones through the speaker.

### audio

Publishes to `hermes-stackchan/jeeves/cmd/audio`.
...
6. Publish valid actions to `hermes-stackchan/jeeves/cmd/*`.
...
spoken filler.
Do not send spoken filler like "Just a moment" unless the user explicitly asks for that
...
things like "remind me in two minutes" or "notify me tomorrow".
...
When the user only says "remind me" without time or content, do not create
a reminder. Ask what and when, set `follow_up_listen: true`, and wait for the
answer.

### system

Publishes to `hermes-stackchan/jeeves/cmd/system`.
...
StackChan also publishes realtime audio events to `hermes-stackchan/jeeves/events`, for example `wakeword_detected`, `recording_started`, and `recording_stopped`.

Hermes HTTP responses consumed by the bridge must be JSON:

```json
{
  "reply": "Answer briefly and politely.",
  "actions": [
    {"action": "face", "emotion": "happy", "intensity_pct": 70}
  ]
}
